...
// Status config
const STATUS = {
  supported:  { icon: "✅", label: "Supported",           color: "text-emerald-600 dark:text-emerald-400", bg: "bg-emerald-50 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-800/40" },
  tentative:  { icon: "⚠️", label: "Tentative — CO Needed", color: "text-amber-600 dark:text-amber-400",   bg: "bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800/40"   },
  conflict:   { icon: "❌", label: "Conflict",             color: "text-red-500 dark:text-red-400",        bg: "bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800/40"           },
  duplicate:  { icon: "🔄", label: "Already in System",   color: "text-zinc-400",                          bg: "bg-zinc-50 dark:bg-zinc-800/40 border-zinc-200 dark:border-zinc-700"          },
};
...
export function SmartUploadView() {
  ...
  if (stage === "upload") return (
    <div className="space-y-5">
      <div className="bg-white dark:bg-zinc-700 border border-zinc-200 dark:border-zinc-600 rounded-xl p-6">
        <div className="flex items-center gap-3 mb-6">
          <div className="w-8 h-8 rounded-full bg-amber-100 dark:bg-amber-900/30 flex items-center justify-center text-amber-600 font-bold text-sm">1</div>
          <div>
            <p className="font-semibold text-zinc-900 dark:text-white">Smart Invoice Upload</p>
            <p className="text-xs text-zinc-400 mt-0.5">AI parses the PDF · Review changes · Approve to save</p>
          </div>
        </div>

        {parseError && (
          <div className="mb-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/40 rounded-lg px-4 py-3 text-xs text-red-600 dark:text-red-400">
            <strong>Parse failed:</strong> {parseError}
          </div>
        )}

        <div
          className={`border-2 border-dashed rounded-xl p-12 text-center cursor-pointer transition-colors ${dragOver ? "border-amber-400 bg-amber-50 dark:bg-amber-900/10" : "border-zinc-200 dark:border-zinc-600 hover:border-amber-300 dark:hover:border-amber-700"}`}
        ...
        >
          ...
          <p className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">Drop Taconic invoice PDF here</p>
          <p className="text-xs text-zinc-400 mt-1">or click to browse · PDF only</p>
        </div>
        ...
        <div className="mt-4 grid grid-cols-3 gap-3 text-center">
          {[["🤖", "AI Reads PDF", "Claude extracts all line items, COs, and amounts automatically"],
            ...
            <div key={title} className="bg-zinc-50 dark:bg-zinc-800/50 rounded-lg p-3">
              <div className="text-lg mb-1">{icon}</div>
              <p className="text-xs font-semibold text-zinc-700 dark:text-zinc-300">{title}</p>
  ...
  if (stage === "parsing") return (
    <div className="bg-white dark:bg-zinc-700 border border-zinc-200 dark:border-zinc-600 rounded-xl p-12 text-center">
      <div className="w-10 h-10 border-2 border-amber-500 border-t-transparent rounded-full animate-spin mx-auto mb-4" />
      <p className="font-semibold text-zinc-800 dark:text-zinc-200">Parsing invoice...</p>
      <p className="text-xs text-zinc-400 mt-1">{pendingFile?.name}</p>
      <p className="text-xs text-zinc-300 dark:text-zinc-600 mt-3">Claude is reading all line items, change orders, and amounts</p>
    </div>
  );
  ...
  if (stage === "review") return (
    <div className="space-y-4">
      {/* Summary bar */}
      <div className="bg-white dark:bg-zinc-700 border border-zinc-200 dark:border-zinc-600 rounded-xl p-4">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            <p className="font-semibold text-zinc-900 dark:text-white">Review: {pendingFile?.name}</p>
            <p className="text-xs text-zinc-400 mt-0.5">Invoice #{parsed?.header?.invNum} · Period to {parsed?.header?.periodTo}</p>
          </div>
          <div className="flex gap-3 text-xs">
            <span className="flex items-center gap-1.5 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 px-2.5 py-1 rounded-full font-medium">✅ {supportedCount} supported</span>
            {tentativeCount > 0 && <span className="flex items-center gap-1.5 bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 px-2.5 py-1 rounded-full font-medium">⚠️ {tentativeCount} tentative</span>}
            {conflictCount > 0 && <span className="flex items-center gap-1.5 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 px-2.5 py-1 rounded-full font-medium">❌ {conflictCount} conflict</span>}
          </div>
        </div>

        {tentativeCount > 0 && (
          <div className="mt-3 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/40 rounded-lg px-3 py-2.5 text-xs text-amber-700 dark:text-amber-400">
      ...
      {Object.entries(sections).map(([sectionName, items]) => (
        <div key={sectionName} className="bg-white dark:bg-zinc-700 border border-zinc-200 dark:border-zinc-600 rounded-xl overflow-hidden">
          <div className="px-4 py-3 border-b border-zinc-100 dark:border-zinc-600 flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-widest text-zinc-400">{sectionName}</span>
          ...
          <div className="divide-y divide-zinc-50 dark:divide-zinc-600/50">
            {items.map(item => {
              ...
              return (
                <div key={item.id} className={`px-4 py-3 flex items-start gap-3 ${item.approved && !item.informational ? "bg-emerald-50/30 dark:bg-emerald-900/5" : ""}`}>
                  {/* Checkbox — only for approvable items */}
                  <div className="pt-0.5 shrink-0">
                    {item.canToggle ? (
                      <button
                        onClick={() => toggle(item.id)}
                        className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${item.approved ? "bg-emerald-500 border-emerald-500 text-white" : "border-zinc-300 dark:border-zinc-600 hover:border-amber-400"}`}
                      >
                        {item.approved && <span className="text-white text-xs font-bold">✓</span>}
                      </button>
                    ) : item.informational ? (
                      <span className="text-zinc-300 dark:text-zinc-600 text-sm">ℹ️</span>
                    ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <p className="text-xs font-semibold text-zinc-800 dark:text-zinc-200">{item.label}</p>
                      <span className={`text-xs font-medium px-2 py-0.5 rounded-full border shrink-0 ${s.bg} ${s.color}`}>{s.label}</span>
                    </div>
                    <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">{item.details}</p>
                    {item.extraDetails?.map((d, i) => (
                      <p key={i} className="text-xs text-zinc-400 dark:text-zinc-500 mt-0.5">{d}</p>
                    ))}
                    {item.note && (
                      <p className={`text-xs mt-1 font-medium ${item.locked ? "text-amber-600 dark:text-amber-400" : item.status === "conflict" ? "text-red-500" : "text-zinc-400"}`}>
      ...
      {/* Action bar */}
      <div className="bg-white dark:bg-zinc-700 border border-zinc-200 dark:border-zinc-600 rounded-xl p-4 flex items-center justify-between gap-4">
        <div className="text-xs text-zinc-500">
          <strong className="text-zinc-800 dark:text-zinc-200">{approvedCount}</strong> item{approvedCount !== 1 ? "s" : ""} selected to approve
          {tentativeCount > 0 && <span className="text-amber-600 dark:text-amber-400 ml-2">· {tentativeCount} locked pending CO upload</span>}
        </div>
        <div className="flex gap-2">
          <button onClick={approveAll} className="px-3 py-2 text-xs font-semibold rounded-lg border border-zinc-200 dark:border-zinc-600 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-600 transition-colors">
            Select All Supported
          </button>
          <button onClick={reset} className="px-3 py-2 text-xs font-semibold rounded-lg border border-zinc-200 dark:border-zinc-600 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-600 transition-colors">
            Cancel
          </button>
          <button
            onClick={submitApprovals}
            disabled={approvedCount === 0 || approving}
            className={`px-5 py-2 text-xs font-bold rounded-lg transition-colors ${approvedCount > 0 && !approving ? "bg-amber-600 hover:bg-amber-500 text-white shadow-sm" : "bg-zinc-100 dark:bg-zinc-800 text-zinc-400 cursor-not-allowed"}`}
          >
  ...
  if (stage === "done") return (
    <div className="bg-white dark:bg-zinc-700 border border-zinc-200 dark:border-zinc-600 rounded-xl p-8 text-center">
      <div className="text-4xl mb-3">✅</div>
      <p className="font-bold text-zinc-900 dark:text-white text-lg">Invoice saved successfully</p>
      ...
      {doneResults?.some(r => !r.ok) && (
        <div className="text-left bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/40 rounded-lg px-4 py-3 mb-4 text-xs text-red-600 dark:text-red-400">
      ...
      <div className="flex gap-3 justify-center">
        <button onClick={reset} className="px-5 py-2.5 bg-amber-600 hover:bg-amber-500 text-white text-xs font-bold rounded-lg transition-colors">
